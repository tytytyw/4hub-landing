...
function CopyLinkFolder({
	...
	const [review] = useState({ text: "Просмотр" });
	...
	const checkContextMenu = (e) => {
		if (!context) {
			e.nativeEvent.path.forEach((el) => {
				...
				if (
					typeof el.className === "string" &&
					el.className.includes(styles.openList)
				)
					setContext("access");
			});
		} else {
			...
			e.nativeEvent.path.forEach((el) => {
				if (
					...
					(el.className.includes(styles.contactsList) ||
						el.className.includes("reviewOption"))
				)
					block = true;
			});
			if (!block) setContext("");
	...
	const renderView = () => (
		<div className={styles.reviewOptions}>
			...
			>
				...
				<div className={styles.description}>Скачивание</div>
			</div>
			<div
			...
			>
				...
				<div className={styles.description}>Редактировать</div>
			</div>
							...
							<div className={`${styles.folder}`}>
								<FolderIcon
									className={`${styles.folderIcon} ${
										colors.filter((el) => el.color === safe.id_color)[0]?.name
									}`}
								/>
								<div className={styles.folderInfo}>{safe.name}</div>
							</div>
