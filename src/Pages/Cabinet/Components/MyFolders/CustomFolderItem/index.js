...
const CustomFolderItem = ({
  ...
  const open = (isOpen) => {
    ...
    const folderWidth = foldersWidth ?? (offDispatch ? 360 : 310) + (open ? p * (f.path?.split("/").length - 1) : 0);
            ...
            {!listCollapsed && (
              <div className={styles.nameWrap}>
                <div className={styles.Name}>
                  <div className={styles.name}>{folderParams.isGlobal ? f?.nameRu : f?.name}</div>
            ...
            {isRecent ? null : (
              <PlayIcon
                className={`${styles.playButton} ${
                  fileList?.path?.includes(f.path) && folderParams.open ? styles.revert : undefined
                }`}
              />
            )}
            {offDispatch ? null : (
      ...
      {isRecent ? null : (
        <div
          ...
          className={`${styles.innerFolders} ${
            fileList?.path?.includes(f.path) && folderParams.open ? undefined : styles.hidden
          }`}
        >
          {offDispatch ? null : (
            <div
