...
function PreviewWithComment() {
  ...
  const toggleComment = () => {
    ...
    return setParams((s) => ({ ...s, comments: c }));
  };

  const renderGroup = (obj) => {
    return Object.keys(obj).map((key) => (
      <div key={key} className={styles.periodWrap}>
        <div className={styles.period}>{periods[key]}</div>
        <div className={styles.commentList}>{renderComments(obj[key])}</div>
      </div>
    ));
  };
  ...
  return (
    <PopUp set={closeModal}>
      <div className={styles.wrap}>
        <div className={styles.imagePanel}>{renderImages()}</div>
