  ...
  const sendFile = async (file) => {
    if (file) {
      ...
      data.append(
        ...
        `${
          file?.options?.name
            ? file.options.name.slice(0, file.options.name.lastIndexOf("."))
    ...
    return list.map((item, i) => {
      return (
        <LoadItem
          ...
          name={
            item?.options?.name
              ? item.options.name.slice(0, item.options.name.lastIndexOf("."))
              : item.file.name.slice(0, item.file.name.lastIndexOf("."))
          }
          ext={
            item?.options?.name
              ? item.options.name.slice(item.options.name.lastIndexOf("."))
    ...
    window.innerHeight / 2 >= params.y
      ...
      : (position.top =
          params.y + (params.height - params.offsetY) < window.innerHeight - 50
            ...
            : `${window.innerHeight - 50 - params.height}px`);
    return position;
  };
            ...
            <div className={styles.progressBarWrap}>
              {collapsed && processing ? (
                <>
                  ...
                  <img
                    src={imageSrc + "assets/PrivateCabinet/download_arrow.svg"}
                    alt=""
                ...
                {timeLeft !== undefined ? (
                  <span className={styles.time}>
                    {timeLeft < 1
                      ...
                      : timeLeft > 59
                      ? __(`Осталось около 1 часа`)
                      : __(`Осталось ${timeLeft} мин.`)}
                  </span>
                ) : null}
              </div>
              <span
