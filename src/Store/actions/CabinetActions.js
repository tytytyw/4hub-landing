...
export const onGetDevices = (setDevicesListLoading, setErrors) => async (
  dispatch,
  getState
) => {
...
export const onGetConnectedContacts = (
  setConnectedContactsListLoading,
  setErrors
) => async (dispatch, getState) => {
  try {
...
// Chat
export const onGetChatGroups = updateGroupUsersList => async (
  dispatch,
  getState
) => {
  const uid = getState().user.uid;
...
export const onGetChatMessages = (target, search, page, loadingMessages) => (
  dispatch,
  getState
) => {
  const uid = getState().user.uid;
...
export const onGetCompanyDocument = (type, loader) => async (
  dispatch,
  getState
) => {
  const uid = getState().user.uid;
...
export const onDeleteCompanyDocument = (type, success, msg) => async (
  dispatch,
  getState
) => {
  const uid = getState().user.uid;
